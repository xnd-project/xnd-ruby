...
class TestTuple < Minitest::Test
  ...
  def test_tuple_optional_values
    ...
    x = XND.new(lst, type: "3 * (?int64, ?int64, ?int64)")
    assert_equal x.value, lst
    assert_equal x.type, NDT.new("3 * (?int64, ?int64, ?int64)")
  end
...
class TestRecord < Minitest::Test
  ...
  def test_record_optional_values
    ...
    assert_equal x.value, lst
    check_copy_contiguous x
    assert_equal x.dtype, NDT.new("{a: ?int64, b: ?int64, c: ?int64}")
  end
...
class TestUnion < Minitest::Test
  ...
  def test_union_optional_values
    # FIXME: data_shapes has to work with this one.                             
  ...
  def test_union_equality
    ...
    # FIXME: adapt data_shapes to this.
    # Test corner cases and many dtypes.
...
class TestTypeInference < Minitest::Test
  def test_data_shape_extraction
    test_cases = [
      [nil, [nil], []],
      [1, [1], []],
      [XND::T.new([1, 2]), [XND::T.new([1, 2])], []],
      [[], [], [[0]]],
      [[nil], [nil], [[1]]],
      [[1], [1], [[1]]],
      [[XND::T.new([])], [XND::T.new([])], [[1]]],
      [[XND::T.new([1, 2])], [XND::T.new([1, 2])], [[1]]],
      [[[]], [], [[0], [1]]],
      [[[nil]], [nil], [[1], [1]]],
      [[[nil, 1]], [nil, 1], [[2], [1]]],
      [[[1, nil]], [1, nil], [[2], [1]]],
